...
var Jl = ye("Raydium_Clmm"),
  ...
  Re = class {
    ...
    static makeSwapBaseInInstructions({
      ...
      console.log("programId:", e.programId);
      console.log("poolId:", e.id);
      let [l, m] = [new v.PublicKey(e.programId), new v.PublicKey(e.id)];
      console.log("vault.A:", t.vault.A, "vault.B:", t.vault.B);
      let [d, p] = [new v.PublicKey(t.vault.A), new v.PublicKey(t.vault.B)];
      console.log("mintA.address:", e.mintA.address, "mintB.address:", e.mintB.address);
      let [y, f] = [
          new v.PublicKey(e.mintA.address),
          new v.PublicKey(e.mintB.address),
      ];
      console.log("config.id:", e.config.id);
      console.log("owner.wallet:", o.wallet?.toBase58());
      console.log("tokenAccountA:", (o.tokenAccountA)?.toBase58());
      console.log("tokenAccountB:", (o.tokenAccountB)?.toBase58());
      console.log("observationId:", n?.toBase58());
      console.log("tickArrayBitmapExtension:", We(l, m).publicKey?.toBase58());
      let b = e.mintA.address === r.toString();
      let g = [
...
var $o = class extends Fe {
  ...
  async swap(e) {
    ...
    let {
      ...
      g = this.createTxBuilder(p),
      [w, P] = [
        new Y.PublicKey(t.mintA.address),
        new Y.PublicKey(t.mintB.address),
      ];
    r
      ? (a.sourceAmountSwapped = a.sourceAmountSwapped
          .mul(new qe.default((1 + c) * 1e4))
          .div(new qe.default(1e4)))
      : (a.destinationAmountSwapped = a.destinationAmountSwapped
          .mul(new qe.default((1 - c) * 1e4))
          .div(new qe.default(1e4)));
    let T = t.mintA.address === $.toBase58(),
      B = t.mintB.address === $.toBase58(),
      { account: h, instructionParams: x } =
        await this.scope.account.getOrCreateTokenAccount({
          mint: w,
          tokenProgram: new Y.PublicKey(
            (C = t.mintA.programId) != null ? C : ut.TOKEN_PROGRAM_ID,
          ),
          owner: this.scope.ownerPubKey,
          createInfo:
            T || !o
          ...
          notUseTokenAccount: T,
          skipCloseAccount: !T,
          associatedOnly: T ? !1 : b,
          checkCreateATAOwner: f,
        });
    x && g.addInstruction(x);
    let { account: S, instructionParams: K } =
      await this.scope.account.getOrCreateTokenAccount({
        mint: P,
        tokenProgram: new Y.PublicKey(
          (L = t.mintB.programId) != null ? L : ut.TOKEN_PROGRAM_ID,
        ),
        owner: this.scope.ownerPubKey,
        createInfo:
          B || o
        ...
        notUseTokenAccount: B,
        skipCloseAccount: !B,
        associatedOnly: B ? !1 : b,
        checkCreateATAOwner: f,
      });
    K && g.addInstruction(K),
      (!h || !S) &&
        this.logAndCreateError("user do not have token account", {
          ...
          mintATokenAcc: h,
          mintBTokenAcc: S,
          mintAUseSOLBalance: T,
          mintBUseSOLBalance: B,
          associatedOnly: b,
        });
    ...
    return (
        ...
        instructions: [
          r
            ? Fu(
                ...
                new Y.PublicKey(
                  (W = t[o ? "mintA" : "mintB"].programId) != null
                    ? W
                    : ut.TOKEN_PROGRAM_ID,
                ),
                new Y.PublicKey(
                  (D = t[o ? "mintB" : "mintA"].programId) != null
                    ? D
                    : ut.TOKEN_PROGRAM_ID,
                ),
                o ? w : P,
                o ? P : w,
                $i(new Y.PublicKey(t.programId), new Y.PublicKey(t.id))
                  .publicKey,
            ...
            : jo(
                ...
                new Y.PublicKey(I.vault[o ? "B" : "A"]),
                new Y.PublicKey(
                  (O = t[o ? "mintA" : "mintB"].programId) != null
                    ? O
                    : ut.TOKEN_PROGRAM_ID,
                ),
                new Y.PublicKey(
                  (F = t[o ? "mintB" : "mintA"].programId) != null
                    ? F
                    : ut.TOKEN_PROGRAM_ID,
                ),
                o ? w : P,
      ...
      g.addCustomComputeBudget(l),
      g.addTipInstruction(m),
      g.versionBuild({ txVersion: d })
    );
  }
  async lockLp(e) {
    var d, p, y, f, b;
